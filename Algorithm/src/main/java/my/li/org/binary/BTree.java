...
public class BTree {
    ...
    private class BTNode {
        // 节点当前关键字个数
        public int n = 0;
        public List<String> keys = new ArrayList<String>(maxKeyNum);
    ...
    private void insertNoFull(BTNode x, String key) {
        int i = x.n - 1;
        if (x.isLeaf) {
    ...
    private void delete(BTNode x, String key) {
        ...
        if (i < n && key.equals(x.keys.get(i))) {
            if (x.isLeaf) {
                ...
                if (y.n >= t) {
                    // 2.a
                    String preKey = deleteMaxKey(y);
                    x.keys.set(i, preKey);
                } else if (z.n >= t) {
                    // 2.b
                    String nextKey = deleteMinKey(z);
            ...
            if (child.n >= t) {
                ...
                String leftBrotherLastKey = leftBrother.keys.get(leftBrotherKeyNum - 1);
                ...
                String rightBrotherFirstKey = rightBrother.keys.get(0);
    ...
    private String deleteMinKey(BTNode x) {
        if (x.isLeaf) {
    ...
    private String deleteMaxKey(BTNode x) {
        int keyNum = x.n;
        if (x.isLeaf) {
